...
def plot_sequence(sequence,
                  show_figure=True):
  """Creates an interactive pianoroll for a tensorflow.NoteSequence.
  ...
  Args:
     sequence: A tensorflow.NoteSequence.
...
def download_bundle(bundle_name, target_dir, force_reload=False):
  ...
  if not os.path.exists(bundle_target) or force_reload:
    response = urllib.request.urlopen(
        'http://download.tensorflow.org/models/%s' % bundle_name)
    data = response.read()
