...
class CycleGAN(object):
    ...
    def _build_model(self, args):
        ...
        # tf.train.CheckpointManager.latest_checkpoint
    ...
    def train(self, args):
        ...
        if self.model == 'full':
            other_set = set(os.listdir('../datasets/')) - set([self.dataset_A_dir, self.dataset_B_dir])
            data_mixed = []
            for i in other_set:
                data_mixed += glob('../datasets/{}/*/*.*'.format(i))
    ...
    def sample_model(self, samples, sample_dir, epoch, idx):
        ...
        # save_npys(samples[0], './{}/A2B/{:02d}_{:04d}_origin.mid'.format(sample_dir, epoch, idx))
        # save_npys(samples[1], './{}/A2B/{:02d}_{:04d}_transfer.mid'.format(sample_dir, epoch, idx))
        # save_npys(samples[2], './{}/A2B/{:02d}_{:04d}_cycle.mid'.format(sample_dir, epoch, idx))
        # save_npys(samples[3], './{}/B2A/{:02d}_{:04d}_origin.mid'.format(sample_dir, epoch, idx))
        # save_npys(samples[4], './{}/B2A/{:02d}_{:04d}_transfer.mid'.format(sample_dir, epoch, idx))
        # save_npys(samples[5], './{}/B2A/{:02d}_{:04d}_cycle.mid'.format(sample_dir, epoch, idx))
    ...
    def test(self, args):
        ...
        ckpt = tf.train.get_checkpoint_state(os.path.join(args.test_model_dir, 'cyclegan.model'))
        print(os.path.join(args.test_model_dir, 'cyclegan.model'))
        # if self.checkpoint.restore(self.checkpoint_manager.latest_checkpoint):
        #     print(" [*] Load checkpoint succeeded!", self.checkpoint_manager.latest_checkpoint)
        # else:
        #     print(" [!] Load checkpoint failed...")
        restore_result = self.checkpoint.restore(ckpt.model_checkpoint_path)
        ...
        for idx in range(len(sample_files)):
            ...
            # save_npys(origin, midi_path_origin)
            # save_npys(transfer, midi_path_transfer)
            # save_npys(cycle, midi_path_cycle)
